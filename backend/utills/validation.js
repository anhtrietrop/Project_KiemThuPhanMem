...
const orderValidation = {
  ...
  validateAddress: (address, fieldName = 'address') => {
    ...
    // For city field, allow more characters including numbers and common city name patterns
    if (fieldName === 'city') {
      // Allow letters, spaces, hyphens, apostrophes, dots, and numbers for city names
      if (!/^[\p{L}\p{M}\s\-'.0-9]+$/u.test(trimmedAddress)) {
        throw new ValidationError(`${fieldName} contains invalid characters`, fieldName);
      }
    }
...
// Comprehensive order validation - Updated to remove company, country, postalCode
