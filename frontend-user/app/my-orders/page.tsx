...
const MyOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const userOrders =
        data?.orders?.filter(
          (order: Order) => order.email === session.user?.email
        ) || [];
  ...
  const getPaymentStatusText = (paymentStatus?: string) => {
    switch (paymentStatus) {
      ...
      default:
        return "⏳ Chờ thanh toán"; // Default to PENDING for old orders
    }
  };
              ...
              <div key={order.id} className="border rounded-lg p-6 bg-gray-50">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <div className="mt-2">
                      <span
                        className={`px-3 py-1 rounded-md text-xs font-semibold border ${getPaymentStatusColor(
