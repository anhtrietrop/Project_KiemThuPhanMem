...
describe('Authorization - Admin vs User Access Control', () => {
  ...
  describe('Admin Access Rights', () => {
    ...
    it('TC-AUTH-13: Admin có thể truy cập Admin Dashboard', () => {
      ...
      cy.get('body').should('contain.text', /dashboard|products|orders|manage|quản lý/i)
    })
    ...
    it('TC-AUTH-17: Admin có thể tạo, sửa, xóa products', () => {
      ...
      cy.get('body').then($body => {
        const hasEdit = $body.find('button, a').filter((i, el) => /edit|sửa/i.test(Cypress.$(el).text())).length > 0
        const hasDelete = $body.find('button, a').filter((i, el) => /delete|xóa/i.test(Cypress.$(el).text())).length > 0
    ...
    it('TC-AUTH-18: Admin có thể cập nhật order status', () => {
      ...
      cy.get('body').then($body => {
        const statusButtons = $body.find('select, button').filter((i, el) => {
          ...
          return /status|trạng thái|pending|processing|shipped/i.test(text) || 
  ...
  describe('User Access Rights', () => {
    ...
    it('TC-AUTH-20: User KHÔNG thể truy cập Products Management', () => {
      cy.visit(Cypress.env('adminUrl') + '/products', { failOnStatusCode: false })
      ...
      cy.get('body').then($body => {
        ...
        if (cartButtons.length > 0) {
          ...
          cy.get('body').should('satisfy', ($body) => {
            ...
            return text.includes('cart') || text.includes('giỏ') || text.includes('added')
          })
        }
      })
    })
    ...
    it('TC-AUTH-26: User KHÔNG thể tạo/sửa/xóa products', () => {
      ...
      cy.get('body').then($body => {
        const adminButtons = $body.find('button, a').filter((i, el) => {
          ...
          return text.includes('delete') || text.includes('edit') || 
    ...
    it('TC-AUTH-27: User có thể thêm wishlist', () => {
      ...
      cy.get('body').then($body => {
        const wishlistButtons = $body.find('button, a').filter((i, el) => {
          const $el = Cypress.$(el)
          return /wishlist|yêu thích|favorite|heart/i.test($el.text()) || 
    ...
    it('TC-AUTH-30: Guest có thể search products', () => {
      ...
      cy.get('body').then($body => {
        if ($body.find('input[type="search"], input[placeholder*="search"], input[placeholder*="tìm"]').length > 0) {
          cy.get('input[type="search"], input[placeholder*="search"]').first().type('laptop')
