...
describe('Search Functionality', () => {
  ...
  describe('Basic Search', () => {
    ...
    it('TC-SEARCH-01: Tìm kiếm product theo tên (keyword matching)', () => {
      ...
      cy.get('body').then($body => {
        const searchInput = $body.find('input[type="search"], input[placeholder*="search"], input[placeholder*="tìm"], input[name="search"]')
        ...
        if (searchInput.length > 0) {
          ...
          cy.url().should('satisfy', (url) => {
            return url.includes('search') || url.includes('laptop') || url.includes('q=')
          })
          
          cy.get('body').should('contain.text', /laptop|product|result|kết quả/i)
        }
      })
    })
    ...
    it('TC-SEARCH-02: Tìm kiếm với keyword không tồn tại', () => {
      ...
      cy.get('body').then($body => {
        const searchInput = $body.find('input[type="search"], input[placeholder*="search"], input[name="search"]')
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').should('satisfy', ($body) => {
            ...
            return text.includes('no') || text.includes('not found') || 
                   text.includes('không tìm thấy') || text.includes('empty')
          })
        }
      })
    })
    ...
    it('TC-SEARCH-03: Tìm kiếm với keyword rỗng hiển thị tất cả products', () => {
      ...
      cy.get('body').then($body => {
        const searchInput = $body.find('input[type="search"], input[name="search"]')
      ...
      cy.get('body').then($body => {
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').then(($firstBody) => {
            ...
            cy.visit(Cypress.env('userUrl'))
            cy.get('input[type="search"], input[name="search"]').first().type('laptop{enter}')
  ...
  describe('Search by Category', () => {
    ...
    it('TC-SEARCH-06: Filter products theo category', () => {
      ...
      // Tìm category filter
      cy.get('body').then($body => {
        const categorySelects = $body.find('select[name="category"], select[name="categoryId"]')
        const categoryLinks = $body.find('a').filter((i, el) => /category|danh mục/i.test(Cypress.$(el).text()))
        
        if (categorySelects.length > 0) {
          // Select category đầu tiên (không phải "All")
          cy.get('select[name="category"], select[name="categoryId"]').select(1)
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').then($resultBody => {
            ...
            if (categoryFilter.length > 0) {
              ...
              cy.url().should('satisfy', (url) => {
                return (url.includes('laptop') || url.includes('search')) && 
  ...
  describe('Search by Price Range', () => {
    ...
    it('TC-SEARCH-09: Filter products theo khoảng giá', () => {
      ...
      cy.get('body').then($body => {
        const priceInputs = $body.find('input[name*="price"], input[placeholder*="price"]')
        ...
        if (priceInputs.length >= 2) {
          ...
          // Apply filter
          cy.get('button').contains(/apply|filter|lọc/i).click()
          
          // Verify: Chỉ hiển thị products trong khoảng giá
          cy.url().should('satisfy', (url) => {
            return url.includes('price') || url.includes('min') || url.includes('max')
          })
        } else if (priceSlider.length > 0) {
          // Sử dụng price slider
          cy.get('input[type="range"]').first().invoke('val', 2000000).trigger('change')
          cy.wait(500)
        }
      })
    })
    ...
    it('TC-SEARCH-10: Min price không thể lớn hơn max price', () => {
      ...
      cy.get('body').then($body => {
        const priceInputs = $body.find('input[name*="min"], input[name*="max"]')
        ...
        if (priceInputs.length >= 2) {
          ...
          cy.get('body').should('satisfy', ($body) => {
            ...
            return text.includes('invalid') || text.includes('không hợp lệ') ||
                   text.includes('greater') || !text.includes('error')
          })
        }
      })
    })
  ...
  describe('Sort Results', () => {
    ...
    it('TC-SEARCH-11: Sắp xếp products theo giá tăng dần', () => {
      ...
      cy.get('body').then($body => {
        const sortSelect = $body.find('select[name*="sort"], select').filter((i, el) => {
          return /sort|sắp xếp/i.test(Cypress.$(el).text()) || 
                 Cypress.$(el).find('option').filter((j, opt) => /price|giá/i.test(Cypress.$(opt).text())).length > 0
        })
        
        if (sortSelect.length > 0) {
          // Chọn "Price: Low to High"
          cy.wrap(sortSelect.first()).find('option').then($options => {
    ...
    it('TC-SEARCH-12: Sắp xếp products theo giá giảm dần', () => {
      ...
      cy.get('body').then($body => {
        const sortSelect = $body.find('select').filter((i, el) => {
          return Cypress.$(el).find('option').filter((j, opt) => /price|giá/i.test(Cypress.$(opt).text())).length > 0
        })
  ...
  describe('Search Performance', () => {
    ...
    it('TC-SEARCH-15: Search trả về kết quả nhanh (< 3s)', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (searchInput.length > 0) {
          ...
          // Wait for results
          cy.get('body').should('contain.text', /product|result/i).then(() => {
    ...
    it('TC-SEARCH-16: Pagination hoạt động khi có nhiều kết quả', () => {
      ...
      cy.get('body').then($body => {
        const pagination = $body.find('nav[aria-label*="pagination"], .pagination, [class*="pagination"]')
        
        if (pagination.length > 0) {
          // Click trang 2
          cy.get('a, button').contains(/2|next|sau/i).first().click()
      ...
      cy.get('body').then($body => {
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').then(($suggestionBody) => {
            if ($suggestionBody.find('.suggestion, .autocomplete, [class*="suggest"]').length > 0) {
    ...
    it('TC-SEARCH-19: Search với nhiều keywords', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').should('satisfy', ($body) => {
            const text = $body.text().toLowerCase()
            return text.includes('laptop') || text.includes('gaming') || text.includes('asus')
          })
        }
      })
    })
      ...
      cy.get('body').then($body => {
        ...
        if (searchInput.length > 0) {
          ...
          cy.get('body').then(($historyBody) => {
            if ($historyBody.find('.history, .recent, [class*="recent"]').length > 0) {
