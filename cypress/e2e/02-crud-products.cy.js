...
describe('CRUD Operations - Products Management', () => {
  ...
  describe('Create Product', () => {
    ...
    it('TC-CRUD-01: Tạo product mới thành công với đầy đủ thông tin', () => {
      ...
      // Submit form
      cy.get('button[type="submit"]').contains(/save|lưu|tạo|create/i).click()
    ...
    it('TC-CRUD-02: Không thể tạo product với giá âm', () => {
      ...
      cy.get('button[type="submit"]').contains(/save|lưu|tạo/i).click()
    ...
    it('TC-CRUD-03: Không thể tạo product với số lượng âm', () => {
      ...
      cy.get('button[type="submit"]').contains(/save|lưu/i).click()
  ...
  describe('Read/View Products', () => {
    ...
    it('TC-CRUD-05: Hiển thị danh sách products', () => {
      ...
      cy.get('body').then($body => {
        ...
        expect(text).to.satisfy((str) => {
          return str.includes('product') || str.includes('No ') || str.includes('empty')
        })
      })
    })
    ...
    it('TC-CRUD-06: Xem chi tiết product', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (viewButtons.length > 0) {
          ...
          cy.get('body').should('contain.text', /price|quantity|description|giá|số lượng|mô tả/i)
        }
      })
    })
    ...
    it('TC-CRUD-07: Tìm kiếm product theo tên', () => {
      ...
      // Nếu có search box
      cy.get('body').then($body => {
        if ($body.find('input[type="search"], input[placeholder*="search"], input[placeholder*="tìm"]').length > 0) {
          cy.get('input[type="search"], input[placeholder*="search"], input[placeholder*="tìm"]')
            .first()
            .type('laptop')
    ...
    it('TC-CRUD-08: Cập nhật thông tin product thành công', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (editButtons.length > 0) {
          ...
          // Submit
          cy.get('button[type="submit"]').contains(/save|lưu|update|cập nhật/i).click()
    ...
    it('TC-CRUD-10: Cập nhật chỉ một trường và giữ nguyên các trường khác', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (editButtons.length > 0) {
          ...
          cy.get('input[name="title"]').invoke('val').then((oldTitle) => {
            // Chỉ update quantity
  ...
  describe('Delete Product', () => {
    ...
    it('TC-CRUD-11: Xóa product thành công', () => {
      ...
      cy.contains(productToDelete).parents('tr, div').within(() => {
        cy.get('button, a').filter((i, el) => {
      ...
      cy.get('body').then($body => {
        if ($body.find('button').filter((i, el) => /confirm|yes|đồng ý|có/i.test(Cypress.$(el).text())).length > 0) {
    ...
    it('TC-CRUD-12: Hiển thị confirm dialog trước khi xóa', () => {
      ...
      cy.get('body').then($body => {
        ...
        if (deleteButtons.length > 0) {
          ...
          cy.get('body').should('contain.text', /confirm|are you sure|bạn có chắc/i)
        }
      })
    })
      ...
      cy.get('body').then($body => {
        ...
        if (deleteButtons.length > 0) {
          ...
          cy.get('body').then($modal => {
            if ($modal.find('button').filter((i, el) => /cancel|hủy|no|không/i.test(Cypress.$(el).text())).length > 0) {
              cy.contains(/cancel|hủy|no|không/i).click()
              
              // Verify: Số lượng products không đổi
              cy.get('button, a').filter((i, el) => /delete|xóa/i.test(Cypress.$(el).text()))
    ...
    it('TC-CRUD-14: Xóa nhiều products cùng lúc (nếu có chức năng)', () => {
      ...
      cy.get('body').then($body => {
        if ($body.find('input[type="checkbox"]').length > 1) {
          ...
          // Click bulk delete
          cy.get('button, a').filter((i, el) => {
            return /delete selected|xóa đã chọn|bulk delete/i.test(Cypress.$(el).text())
          }).click()
          
          // Confirm
          cy.get('body').then($modal => {
            if ($modal.find('button').filter((i, el) => /confirm|yes|đồng ý/i.test(Cypress.$(el).text())).length > 0) {
