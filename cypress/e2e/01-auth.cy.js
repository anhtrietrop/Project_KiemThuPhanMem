...
describe('Authentication - Login & Register', () => {
  ...
  describe('User Login', () => {
    ...
    it('TC-AUTH-01: Đăng nhập thành công với tài khoản User hợp lệ', () => {
      ...
      cy.get('input[name="email"], input[type="email"]').should('be.visible').type('user@example.com')
      cy.get('input[name="password"], input[type="password"]').should('be.visible').type('user123')
      ...
      cy.get('body').should('contain.text', /logout|đăng xuất|profile|tài khoản/i)
    })
    ...
    it('TC-AUTH-02: Đăng nhập thất bại với email sai', () => {
      ...
      cy.get('body').should('contain.text', /invalid|incorrect|sai|không tồn tại/i)
  ...
  describe('Admin Login', () => {
    ...
    it('TC-AUTH-05: Admin đăng nhập thành công vào Admin Panel', () => {
      ...
      cy.get('body').should('contain.text', /dashboard|products|orders|quản lý/i)
    })
  ...
  describe('User Registration', () => {
    ...
    it('TC-AUTH-07: Đăng ký thành công với thông tin hợp lệ', () => {
      ...
      cy.get('body').then($body => {
        if ($body.find('input[name="name"], input[name="username"]').length > 0) {
          cy.get('input[name="name"], input[name="username"]').type('Test User')
        }
      })
      
      cy.get('button[type="submit"]').contains(/đăng ký|register|sign up/i).click()
    ...
    it('TC-AUTH-08: Đăng ký thất bại với email đã tồn tại', () => {
      ...
      cy.get('button[type="submit"]').contains(/đăng ký|register/i).click()
    ...
    it('TC-AUTH-09: Đăng ký thất bại với mật khẩu không khớp', () => {
      ...
      const timestamp = Date.now()
      cy.get('input[name="email"], input[type="email"]').type(`test${timestamp}@example.com`)
      cy.get('input[name="password"], input[type="password"]').first().type('Password123!')
      ...
      cy.get('body').then($body => {
        if ($body.find('input[name="confirmPassword"]').length > 0) {
          ...
          cy.get('body').should('contain.text', /not match|không khớp|do not match/i)
        }
      })
    })
  ...
  describe('Session Management', () => {
    ...
    it('TC-AUTH-11: Logout thành công', () => {
      ...
      cy.get('body').should('not.contain.text', /my account|tài khoản của tôi/i)
    })
