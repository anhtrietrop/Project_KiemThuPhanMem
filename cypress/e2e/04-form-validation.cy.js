...
describe("Form Validation", () => {
  ...
  describe("Login Form Validation", () => {
    ...
    it("TC-VAL-03: Password field bắt buộc nhập", () => {
      ...
      cy.get('input[name="password"], input[type="password"]').then(
        ($input) => {
          expect($input[0].validationMessage).to.not.be.empty;
        }
      );
    ...
    it("TC-VAL-04: Hiển thị lỗi khi email không tồn tại", () => {
      ...
      cy.get("body").should(
        "contain.text",
        /invalid|incorrect|not found|không tồn tại/i
      );
    });
  ...
  describe("Register Form Validation", () => {
    ...
    it("TC-VAL-09: Password phải có độ dài tối thiểu", () => {
      ...
      cy.get("body").should(
        "contain.text",
        /too short|quá ngắn|at least|ít nhất/i
      );
    });
    ...
    it("TC-VAL-10: Confirm password phải khớp với password", () => {
      ...
      cy.get("body").then(($body) => {
        if ($body.find('input[name="confirmPassword"]').length > 0) {
          ...
          cy.get("body").should(
            "contain.text",
            /not match|không khớp|mismatch/i
          );
        }
      });
    });
  ...
  describe("Product Form Validation (Admin)", () => {
    ...
    it("TC-VAL-12: Title bắt buộc nhập", () => {
      ...
      cy.get('input[name="title"]').then(($input) => {
        expect(
          $input[0].validationMessage || $input[0].validity.valid === false
        ).to.exist;
      });
    });
    ...
    it("TC-VAL-14: Price không thể là 0", () => {
      ...
      cy.get("body").should(
        "contain.text",
        /greater than|lớn hơn|invalid|không hợp lệ/i
      );
    });
  ...
  describe("Checkout Form Validation", () => {
    beforeEach(() => {
      ...
      cy.get("body").then(($body) => {
        const addCartBtns = $body
          .find("button")
          .filter((i, el) => /add to cart|thêm/i.test(Cypress.$(el).text()));
    ...
    it("TC-VAL-19: Name không được để trống", () => {
      cy.get("body").then(($body) => {
        if ($body.find('input[name="name"]').length > 0) {
          ...
          cy.get('input[name="adress"], input[name="address"]').type(
            "123 Street"
          );
          cy.get('button[type="submit"]').click();
    ...
    it("TC-VAL-20: Phone phải đúng format số điện thoại", () => {
      cy.get("body").then(($body) => {
        if ($body.find('input[name="phone"]').length > 0) {
          ...
          cy.get("body").should(
            "contain.text",
            /invalid|không hợp lệ|phone|số điện thoại/i
          );
        }
      });
    });
    ...
    it("TC-VAL-21: Address không được để trống", () => {
      cy.get("body").then(($body) => {
        if (
          $body.find('input[name="adress"], input[name="address"]').length > 0
        ) {
          cy.get('input[name="name"]').type("Test User");
          cy.get('input[name="phone"]').type("0123456789");
          // Bỏ trống address
          ...
          cy.get('input[name="adress"], input[name="address"]').then(
            ($input) => {
              expect($input[0].validationMessage).to.not.be.empty;
            }
          );
    ...
    it("TC-VAL-27: Password strength indicator (nếu có)", () => {
      ...
      cy.get("body").then(($body) => {
        if ($body.find('.password-strength, [class*="strength"]').length > 0) {
          cy.get('.password-strength, [class*="strength"]').should(
            "be.visible"
          );
        }
      });
